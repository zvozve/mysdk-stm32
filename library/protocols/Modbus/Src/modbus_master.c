...
static int master_request_async(modbus_t *ctx, uint8_t func_code,
    ...
    if (ret != 0) {
        MODBUS_LOG("[TX-FAIL] ret=%d (frame NOT on bus)", ret);
...
typedef struct {
    ...
    uint16_t tx_fail_count;            /* 连续发送失败计数（TX_BUSY 等），达阈值→断线收敛 */
} mb_arbiter_t;
...
static int mb_dispatch(modbus_t *ctx, mb_arbiter_t *a, uint8_t kind, void *ptr,
    ...
    /* ★ 无论成功失败都记账 last_send_tick：失败也压回 min_frame_gap 节拍，
     * 避免"send 恒失败→状态机停在 IDLE→last_send_tick 不更新→帧间隔闸门
     * 永远放行→1ms 狂发"（见 2026-08-31 日志）。 */
    ...
    if (ret != 0) {
        ...
        /* 连续发送失败收敛：达阈值直接置 DISCONNECTED（对称于 RX 超时兆底），
         * 让仲裁器开头 return、彻底静黛，避免死链 1ms 刷屏。 */
        if (++a->tx_fail_count >= 3) {
            a->tx_fail_count = 0;
            ctx->line_state = MODBUS_LINE_DISCONNECTED;
            ctx->reconnect_tick = ctx->transport.get_tick();
            MODBUS_LOG("TX fail x3, line DISCONNECTED!");
