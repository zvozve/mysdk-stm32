...
#define	RAM_ENDP0_SZ16		0x10		/* 16字节数据长度模式下端点0的缓冲区长度 */
#define	RAM_ENDP0_BUF		0x20		/* 端点0接收和发送共用16字节缓冲区的起始地址 */
...
#define	REG_SYS_AUX			0x01		/* 系统辅助设置寄存器 */
...
#define	BIT_AUX_HW_XID		0x03		/* 硬件识别位的位反: 总是常量10, 否则说明读操作或硬件连接有误 */
#define	BIT_EP0_16BYTES		0x04		/* 端点0支持最大16字节数据长度模式：0=最大8字节，收发缓冲区独立；1=最大16字节，收发缓冲区共用 */
#define	BIT_GP_BIT			0x08		/* 通用位变量，可以软件定义 */
